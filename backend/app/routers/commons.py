...
from app.schemas.commons import (DataAll, brcName, IsertStatus, IsertStatus2, Dataitem, imageAll, MassStatus, IsertFinish, Qtydata, Dataptinfo, Dataptinfo2, DataAll_dentallight, AuthorInfo)
...
###################################################################################
    @router.get(
        "/get_ptinfo",
        response_model=List[Dataptinfo],
        dependencies=[Depends(api_key_auth)],
    )
    async def get_ptinfo(
        item_id: int,
        db: AsyncSession = Depends(db),
    ):
        try:
            data_all = await manager.get_ptinfo(
                item_id=item_id,
                db=db,
            )
            return list(data_all)
...
###################################################################################
    @router.get(
        "/get_ptinfo2",
        response_model=List[Dataptinfo2],
        dependencies=[Depends(api_key_auth)],
    )
    async def get_ptinfo2(
        item_id: int,
        db: AsyncSession = Depends(db),
    ):
        try:
            data_all = await manager.get_ptinfo2(
                item_id=item_id,
                db=db,
            )
            return list(data_all)
...
###################################################################################
    ...
    async def post_status_QR(item:IsertStatus, db: AsyncSession = Depends(db)):
        print("hello",item)
        try:
            post_status_QR = await manager.post_status_QR(item=item, db=db)
            return {"success": True}
        except Exception as e:
            raise HTTPException(
                status_code=400, detail=f"Error during update : {e}"
...
###################################################################################
    ...
    async def save_comment(
        ...
        try:
            save_directory = r"C:\Users\nt\Desktop\Code\asd\WS\backend\comment_image"

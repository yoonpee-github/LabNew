...
###################################################################################
    async def get_all_data(
        ...
        WHERE 
            ...
            AND rcp_item.prd_id IN (53, 54, 55, 56, 57, 71, 72, 122, 126, 231, 283, 307, 314, 338, 339, 357, 378, 403, 404, 405, 406, 407, 408, 409, 410, 411, 412, 421, 422, 423)
...
###################################################################################
    async def get_ptinfo(self, item_id: int, db: AsyncSession):
        try:
            stmt = text("""
            SELECT
                rcp_item.item_id,
            ...
            FROM 
                rcp_info
            LEFT JOIN pt_info ON rcp_info.pt_id = pt_info.pt_id
            LEFT JOIN rcp_item ON rcp_info.rcp_id = rcp_item.rcp_id
            LEFT JOIN (
                SELECT item_id, qty_data, date_of_qty, author_qty
                FROM qty_info q1
                WHERE q1.date_of_qty = (
                    SELECT MAX(q2.date_of_qty)
                    FROM qty_info q2
                    WHERE q2.item_id = q1.item_id
                )
            ) latest_qty_data ON rcp_item.item_id = latest_qty_data.item_id
            LEFT JOIN 
            (
            ...
            LEFT JOIN (
                SELECT si1.item_id, si1.finish_date
                FROM finishdate_info si1
                WHERE si1.date_of_insertsss = (
                    SELECT MAX(si2.date_of_insertsss)
                    FROM finishdate_info si2
                    WHERE si2.item_id = si1.item_id
                )
            ) finishdate_info ON rcp_item.item_id = finishdate_info.item_id
            LEFT JOIN pt_book ON rcp_info.rcp_id = pt_book.rcp_id
            LEFT JOIN sto_prd ON rcp_item.prd_id = sto_prd.prd_id
            LEFT JOIN dr_info ON rcp_item.dr_id = dr_info.dr_id
            LEFT JOIN bsn_branch ON rcp_info.brc_id = bsn_branch.brc_id
            LEFT JOIN (
                SELECT pt_id, date_of_insert, date_of_book, time_of_book_start
                FROM pt_book 
                WHERE date_of_book IS NOT NULL
            ...
            WHERE 
                rcp_item.item_id = :item_id;
            """)
            rs = await db.execute(stmt, {"item_id": item_id})
            return rs
        except Exception as e:
            raise e

###################################################################################
    async def get_ptinfo2(self, item_id: int, db: AsyncSession):
        try:
            stmt = text("""
            SELECT
                rcp_item.item_id,
                ...
                second_query.date_of_book,
                bsn_branch.brc_sname,
                GROUP_CONCAT(status_info.status_data ORDER BY status_info.date_of_inserts ASC) AS status_data,
                finishdate_info.finish_date
            FROM 
                rcp_info
            LEFT JOIN pt_info ON rcp_info.pt_id = pt_info.pt_id
            ...
            WHERE 
                rcp_item.item_id = :item_id
            GROUP BY
                rcp_item.item_id, rcp_info.rcp_id;
            """)
            rs = await db.execute(stmt, {"item_id": item_id})
            return rs
        except Exception as e:
...
###################################################################################
    async def post_status_QR(self, db: AsyncSession, item:IsertStatus):
        try:
            stmt = f"""
            INSERT INTO status_info (item_id, status_data, author_status) 
            VALUES (:item_id, :status_data, :author_status);

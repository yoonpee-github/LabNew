...
from app.schemas.commons import (DataAll, brcName, IsertStatus, IsertStatus2, Dataitem, CommentData, imageAll, MassStatus, IsertFinish, Qtydata, Dataptinfo, Dataptinfo2, DataAll_dentallight, AuthorInfo)
import json
...
###################################################################################
    async def get_ptinfo(
            ...
            res = await self.crud.get_ptinfo(db=db, item_id=item_id)
            return_list = []
...
###################################################################################
    async def get_ptinfo2(
            ...
            res = await self.crud.get_ptinfo2(db=db, item_id=item_id)
            return_list = []
...
###################################################################################
    async def post_status_QR(
        self,
        item: IsertStatus,
        db: AsyncSession = None,
    ):
        print("edit", item)
        await self.crud.post_status_QR(db=db, item=item)
        return True
